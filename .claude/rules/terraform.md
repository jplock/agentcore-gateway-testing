...
- All data sources (including `aws_iam_policy_document`) live in `data.tf` and all locals in `locals.tf`, per directory. Reference account/partition via `local.aws_account_id` / `local.aws_partition` rather than data-source attributes inline.
